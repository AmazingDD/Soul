...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, bias=False):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, bias=bias, groups=groups)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.static:
            x = torch.nn.functional.conv2d(
                x, weight, bias=self.conv.bias, stride=self.conv.stride, padding=self.conv.padding, dilation=self.conv.dilation,groups=self.conv.groups)
            ...
            x = x.flatten(0, 1)
            x = torch.nn.functional.conv2d(
                x, weight, bias=self.conv.bias, stride=self.conv.stride, 
