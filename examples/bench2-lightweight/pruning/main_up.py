...
from resnet import SewResNet18
from pruner import UnstructuredPruner
...
if __name__ == '__main__':
    ...
    ensure_dir(os.path.join(args.output_dir, 'sparse'))
    ...
    pruner = UnstructuredPruner(model_map[args.model](input_shape, args.T, num_classes))
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > max_test_acc:
            ...
            torch.save(model.state_dict(), f'./{args.output_dir}/raw/best_{args.model}_{args.dataset}_T{args.T}_thr{args.threshold}.pth')

    # unstructured pruning
    model.load_state_dict(
        torch.load(f'./{args.output_dir}/raw/best_{args.model}_{args.dataset}_T{args.T}_thr{args.threshold}.pth', map_location='cpu'))
    model.to(device)

    test_acc, _ = testing(model, test_loader, criterion, device)
    print(f'Before pruning, Test Acc. {test_acc:.2f}%')

    pruner.apply_pruning(ratio=args.threshold)
    pruner.compute_sparsity()
    ...
    for epoch in range(args.ft_epochs):
        ...
        if test_acc > max_test_acc:
            max_test_acc = test_acc
            torch.save(model.state_dict(), f'./{args.output_dir}/sparse/best_sparse_{args.model}_{args.dataset}_T{args.T}_thr{args.threshold}.pth')
